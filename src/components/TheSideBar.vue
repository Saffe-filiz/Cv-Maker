	...
	<aside class="hidden xl:block w-[111px] h-screen bg-[#9fb1db]" ref="container">
		...
		<div class="w-full max-h-[80vh] flex flex-col">
			<button class="w-full h-10 bg-[#9fb1db] flex-none" v-show="currentHeight" >
			    	<svg class="w-6 h-6 mx-auto" :class="{'invisible': status.firstPosition}" fill="#0000008a" viewBox="0 0 24 24">
			...
			<div class="w-full h-auto overflow-y-auto overflow-x-hidden sideNav" ref="sideList">
			    <ul class="w-full  flex flex-col items-center gap-y-[5px]">
				    <li class="w-[94px] h-20 rounded flex justify-center flex-col items-center group cursor-pointer" :class="{'bg-[#e7ecf9]': true}" ref="item">
					<span >
				    ...
				    <li class="w-[94px] h-20  rounded flex justify-center flex-col items-center group cursor-pointer" :class="{'bg-[#e7ecf9]': currentIndex}">
					<span >
			...
			<button class="w-full h-10 bg-[#9fb1db]" v-show="currentHeight">
			    <svg class="w-6 h-6 mx-auto transform rotate-180" :class="{'invisible': status.secondPostion}" fill="#0000008a" viewBox="0 0 24 24">
			    	<path d="m12 8-6 6 1.41 1.41L12 10.83l4.59 4.58L18 14z"></path>
			    </svg>
		    </button>
		</div>
...
<script setup>
	...
	onMounted(() => {
		...
		sideList.value.addEventListener('scroll', () => {
			let position = item.value.getBoundingClientRect().top;
			console.log(position)
			status.firstPosition = position > 100
			status.secondPostion = position < 0
		})
	});
