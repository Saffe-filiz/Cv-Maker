<template>
	<aside class="hidden xl:block w-[111px] h-full bg-[#9fb1db]" >
		<div class="h-[70px] w-full"></div>
		<div class="w-full h-full flex flex-col">
			<button class="w-full h-10 bg-[#9fb1db] flex-none" v-show="currentHeight">
			    <svg class="w-6 h-6 mx-auto" fill="#0000008a" viewBox="0 0 24 24">
				    <path d="m12 8-6 6 1.41 1.41L12 10.83l4.59 4.58L18 14z"></path>
			    </svg>
		    </button>
			<div class="w-full h-auto overflow-y-auto overflow-x-hidden sideNav">
			    <ul class="w-full  flex flex-col items-center gap-y-[5px]">
				    <li class="w-[94px] h-20 rounded flex justify-center flex-col items-center group cursor-pointer" :class="{'bg-[#e7ecf9]': true}">
					<span >
			...
			<button class="w-full h-10 bg-[#9fb1db]" v-show="currentHeight">
			    <svg class="w-6 h-6 mx-auto transform rotate-180" fill="#0000008a" viewBox="0 0 24 24">
				    <path d="m12 8-6 6 1.41 1.41L12 10.83l4.59 4.58L18 14z"></path>
			    </svg>
		    </button>
		</div>
...
<script setup>
	...
	let currentHeight = ref(null);

	const  currentIndex = ref(0);

	onMounted(() => window.addEventListener("resize", pageResize))

	const pageResize =  (event) => {
		contentHeight.value = event.target.innerHeight
		console.log(contentHeight.value)
		currentHeight.value = event.target.innerHeight < 720
	}
